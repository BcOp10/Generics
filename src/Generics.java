...
public static <T extends Comparable<T>> ArrayList<Student<T>> createRandomArray(int arrayLength, String typeName){
    ...
    for (int i = 0; i < arrayLength; i++){
        ...
        switch (typeName){
            case "Integer":
            arrayList.add((Student<T>) new Student<Integer>(id, rand.nextInt(100)));
            break;
            case "Float":
            arrayList.add((Student<T>) new Student<Float>(id, rand.nextFloat(100)));
            break;
            case "Double":
            arrayList.add((Student<T>) new Student<Double>(id, rand.nextDouble(100)));
            break;
            default:
